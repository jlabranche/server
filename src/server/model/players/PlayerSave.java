...
public class PlayerSave {
		...
		while (EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("repPoints")) {
						p.repPoints = Integer.parseInt(token2);
					} else if (token.equals("slayPoints")) {
						p.slayPoints = Integer.parseInt(token2);
					} else if (token.equals("rankLevel")) {
	...
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("repPoints = ", 0, 12);
			characterfile.write(Integer.toString(p.repPoints), 0, Integer
					.toString(p.repPoints).length());
			characterfile.newLine();
			characterfile.write("slayPoints = ", 0, 13);
			characterfile.write(Integer.toString(p.slayPoints), 0, Integer
					.toString(p.slayPoints).length());
			characterfile.newLine();
			characterfile.write("rankLevel = ", 0, 12);
			characterfile.write(Integer.toString(p.rankLevel), 0, Integer
					.toString(p.rankLevel).length());
			characterfile.newLine();
