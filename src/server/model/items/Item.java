...
public class Item {
	...
	public static boolean playerCape(int itemId) {
		String[] data = {
			"TokHaar-Kal", "Ava's accumulator","cloak", "cape", " Guthix cloak", "Ava's attractor"
		};
	...
	public static boolean playerBoots(int itemId) {
		String[] data = {
			"Glaiven boots","Ragefire boots","Steadfast boots", "Flippers","Shoes",
			" shoes", "boots", "Boots"
		};
		String item = getItemName(itemId);
		if (item == null) {
	...
	public static boolean playerAmulet(int itemId) {
		String[] data = {
				"Gnome scarf","scarf","Phoenix necklace","necklace","amulet",
				"Amulet", "Pendant", "pendant", "Symbol", "symbol", "Arcane stream",
				"amulet(e)", "Salve Amulet(e)",
		};
		String item = getItemName(itemId);
		if (item == null) {
			return false;
	...
	public static boolean playerHats(int itemId) {
		String[] data = {
			...
			"hat(g)", "hat(t)", "bandana", "Helm", "heraldic helm", "Cap and goggles", "A powdered wig", "cap"
		};
		String item = getItemName(itemId);
		if (item == null) {
			return false;
	...
	public static boolean playerBody(int itemId) {
		String[] data = {
			"body", "top", "plate", "Priest gown", "apron", "shirt", "7390",
			"platebody", "robetop", "body(g)", "body(t)", 
			"Wizard robe (g)", "Wizard robe (t)", "body", "brassard", "blouse", 
			"tunic", "leathertop", "Saradomin plate", "chainbody", "Varrock armour 1",
			"hauberk", "Shirt", "torso", "mail", "chestplate", "Saradomin", "Guthix", "Zamorak", "Bomber jacket", "jacket"
		};
		String item = getItemName(itemId);
		if (item == null) {
			return false;
	...
	private static String[] fullbody = {
		...
		"torso", "hauberk", "Dragon chainbody", "plate", 
		"Rock-shell plate", "Morrigan's leather body", "Bomber jacket"
	};

	private static String[] fullhat = {
		"Wolf mask","Bat mask","Penguin mask","Cat mask","med helm", 
		"coif", "Dharok's helm", "hood", "Initiate helm",
		"Coif","Helm of neitiznot","Armadyl helmet","Berserker helm", 
		"Archer helm", "Farseer helm", "Warrior helm", "Void", 
		"Cap and goggles", "Grim reaper hood", "heraldic helm", "full helm(g)",
	};

	private static String[] fullmask = {
		"Wolf mask","Bat mask","Gas mask","Penguin mask","Cat mask","Fully slayer helmet", 
		"Slayer helmet","Full slayer helmet", "full helm", "Verac's helm", 
		"Guthan's helm", "Karil's coif", "h'ween mask", "Torag's helm", 
		"sallet", "Saradomin helm", "Grim reaper hood", "heraldic helm",
		 "full helm(g)",
	};
